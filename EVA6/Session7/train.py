...
def train(model, device, train_loader, optimizer, l1, scheduler):
    ...
    for batch_idx, (data, target) in enumerate(pbar):
        ...
        pbar.set_description(
            desc=f"Batch_id={batch_idx} Loss={train_loss/num_loops:.5f} Accuracy={100*correct/processed:0.2f}"
        )
...
def fit_mod(net, NUM_EPOCHS=20, l1=False, l2=False, drop=0.025, num_groups=4):
    ...
    scheduler = optim.lr_scheduler.OneCycleLR(
        optimizer, max_lr=0.017, epochs=NUM_EPOCHS, steps_per_epoch=len(train_loader)
    )
